---
siteName: Will Godfrey
title: Venture architect
canonicalHost: https://www.willgodfrey.com
email: will@willgodfrey.com
linkedin: https://www.linkedin.com/in/willgodfrey
location: Iowa City, Iowa
nav:
  - label: Venture architecture
    href: /venture-architect/
  - label: Work
    href: /work/
  - label: About
    href: /about/
  - label: Start a conversation
    href: /contact/
meta:
  home:
    title: Will Godfrey, venture architect
    description: "Venture architect: the architect of record for new AI-native businesses. I design them, prove the design in a working system, and hand over on a date."
  essay:
    title: What is a venture architect? By Will Godfrey
    description: "A definition of the senior role: the architect of record for a new business, and why AI-native ventures need one."
  work:
    title: Work. Will Godfrey, venture architect
    description: Five things I have designed and built, from a corporate venture build to two ventures of my own.
  about:
    title: About Will Godfrey, venture architect
    description: Architect of applications, then of enterprises, then of ventures. Nine years at McKinsey. Author of a method for building AI-native ventures.
  contact:
    title: Start a conversation. Will Godfrey, venture architect
    description: Tell me what you are weighing. I read everything myself.
  notFound:
    title: Page not found. Will Godfrey
share:
  title: Will Godfrey, venture architect
  description: The architect of record for new AI-native businesses. One per venture, working for the owner.
person:
  name: Will Godfrey
  jobTitle: Venture architect
  alumniOf: Georgia Institute of Technology
  knowsAbout:
    - Venture architecture
    - AI-native business design
    - Corporate venture building
    - Enterprise architecture
    - Unit economics of AI products
images:
  portrait:
    alt: "[[TODO: portrait alt text]]"
    caption: "[[TODO: portrait caption, or delete]]"
  aerialHome:
    alt: "[[TODO: aerial 1 alt text]]"
    caption: "[[TODO: aerial 1 caption: place, month, year]]"
  aerialWork:
    alt: "[[TODO: aerial 2 alt text]]"
    caption: "[[TODO: aerial 2 caption: place, month, year]]"
verification: "ICONIQ 2026 State of AI figures verified 2026-09-20"

# Global prose, relocated from the body into typed fields (words unchanged) so the
# shared layout can read it. See docs/OPEN_ITEMS.md for the restructure note.
closingBlock: "Will Godfrey is a venture architect: the architect of record for new AI-native businesses. He spent nine years at McKinsey, where he served as CTO of North America's first large-scale digital business build. He wrote a method for building AI-native ventures and runs two ventures of his own, Sightline Geospatial and Miniature Systems, from Iowa City."
closingLink: Start a conversation
titleBlock:
  practice: Will Godfrey, venture architect
  labels:
    sheet: Sheet
    date: Date
    revision: Revision
    contact: Contact
    elsewhere: Elsewhere
  elsewhere: LinkedIn
notFound:
  headline: This sheet is not in the set
  body: "The page you were looking for does not exist, or it was part of an older version of this site."
  links:
    - label: Home
      href: /
    - label: What is a venture architect?
      href: /venture-architect/
bios:
  short: "Will Godfrey is a venture architect: the architect of record for new AI-native businesses. Nine years at McKinsey. Author of a method for building AI-native ventures."
  long: "Will Godfrey is a venture architect. He designs new AI-native businesses before they are built, proves the design in a working system, and hands it over on a date. He spent nine years at McKinsey, where he served as CTO of North America's first large-scale digital business build. He wrote a method for building AI-native ventures and runs two ventures of his own."
---
